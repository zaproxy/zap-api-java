...
public class ReportTask extends ZapTask {
    ...
    private static final List<String> SUPPORTED_REPORT_TYPES =
            Arrays.asList(HTML_REPORT_TYPE, MD_REPORT_TYPE, XML_REPORT_TYPE);
    ...
    private void validateTaskAttributes() {
        ...
        if (file.isFile()) {
            if (!file.canWrite()) {
                throw new BuildException(
                        "The provided file is not writable: " + file.getAbsolutePath());
            }

            if (!overwrite) {
                throw new BuildException(
                        "The file already exists but 'overwrite' attribute is not set to 'true': "
            ...
            if (dir == null) {
                throw new BuildException(
                        "Unable to determine parent directory of the file provided: "
                                + file.getAbsolutePath());
            }
            ...
            if (!dir.canWrite()) {
                throw new BuildException(
                        "The provided directory does not exist or is not writable: "
                                + dir.getAbsolutePath());
            }
        } else {
            throw new BuildException(
                    "The 'file' attribute does not specify a file: " + file.getAbsolutePath());
        }
    }
}
